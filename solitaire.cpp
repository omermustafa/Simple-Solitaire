...
struct List{
	...
	void run_yap();//Wants user to select choice from menu, and calls related functions
...
int main()
{
	...
	char selection;
	run.create();
	if(!run.fptr)//If cannot find solitaire.txt file, close the program
		return 0;

	run.istopempty = false;//If the top list is empty, this value will be true
	run.isboardempty = false;//If the board list is empty, this value will be true
	...
	while(1){
		...
		if(selection == '1'){
			run.goster_TopToFoundation();
			cin.ignore(10000, '\n');
			getchar();
		}
		else if(selection == '2'){
			run.goster_TopToBoard();
			cin.ignore(10000, '\n');
			getchar();
		}
		else if(selection == '3'){
			run.goster_BoardToBoard();
			cin.ignore(10000, '\n');
			getchar();
		}
		else if(selection == '4'){
			run.goster_BoardToFoundation();
			cin.ignore(10000, '\n');
		}
		else if(selection == '9'){
			run.close();
		...
		if(!run.top_head)// checking top list empty or not
			run.istopempty = true;
		
		List_Node *traverse;
		traverse = run.board_head;
		int counter = 0;
		while(traverse){//checking board lists is empty or not
			...
			if(counter == 7)// counter is 7 if all of 7 boardlists are empty
				run.isboardempty = true;
		}

		/*When game is completed prints message to user*/
		if(run.isboardempty && run.istopempty){//Stop while loop when board and top lists are empty
